...
int main(int argc, char* argv[])
{
    ...
    float r_x = 60.869;     // yaw
    float r_y = -49.4783;   // roll
    float r_z = -3.29358;   // pitch
    ...
    fitting::RenderingParameters rendering_params(current_pose, image.cols, image.rows);
    ...
    // Get image_points, model_points, vertex_indices
    tuple <vector<Vector2f>, vector<Vector4f>, vector<int>> pointset = fitting::get_corresponding_pointset(landmarks, landmark_mapper, morphable_model);
    
    vector<Vector2f> image_points = get<0>(pointset);
    vector<Vector4f> model_points = get<1>(pointset);
    vector<int> vertex_indices = get<2>(pointset);
    
    // Fit the model, get back a combined_shape
    VectorXf combined_shape = fitting::fit_shape(affine_camera_matrix, morphable_model, blendshapes, image_points, vertex_indices, 30.0f, cpp17::nullopt, pca_shape_coefficients, blendshape_coefficients);
